...
bool protectUSBDrive(const std::string& drivePath, const std::string& key) {
    ...
    // Զտել միայն .encrypted չունեցող ֆայլերը
...
bool unprotectUSBDrive(const std::string& drivePath, const std::string& key) {
    ...
    for (size_t i = 0; i < encryptedFiles.size(); i++) {
        std::string originalFile = encryptedFiles[i];
        const std::string suffix = ".encrypted";
        if (originalFile.size() >= suffix.size() &&
            originalFile.compare(originalFile.size() - suffix.size(), suffix.size(), suffix) == 0) {
            originalFile.erase(originalFile.size() - suffix.size());
        }

        std::cout << "Decrypting: " << encryptedFiles[i] << " ... ";

        if (cipher.decryptFile(encryptedFiles[i], originalFile)) {
            // Ջնջել գաղտնագրված ֆայլը
    ...
    // Ջնջել պաշտպանված թղթապանակը (եթե դատարկ է)
    std::string protectedDir = drivePath + "\\Protected";
/*  elder C++17
    if (FileOperations::fileExists(protectedDir)) {
...
int main(int argc, char* argv[]) {
    ...
    if (command == "help") {
        ...
        if (inputFile.size() >= suffix.size() &&
            inputFile.compare(inputFile.size() - suffix.size(), suffix.size(), suffix) == 0) {
            outputFile = inputFile.substr(0, inputFile.size() - suffix.size());
        }
        else {
